...
var pieceSelected = 'url("game/images/misc/legal_selected.png")';
var pieceSource = 'url("game/images/misc/legal_source.png")';

// import sounds
const MOVE_SOUND = new Audio('game/sounds/move.wav');
const CAPTURE_SOUND = new Audio('game/sounds/capture.wav');
var SOUNDTRACK = new Audio('game/sounds/melody-of-nature-main.mp3');
...
function drawBoard() {
  ...
  for (let row = 0; row < 14; row++) {
    ...
    for (let col = 0; col < 11; col++) {
      ...
      var pieceImage = '<img style="width: 42px" draggable="true"';
...
function setBot(bot) {
  ...
  document.getElementById('botName').textContent = bots[bot].description;
}
...
// play soundtrack
function playSoundTrack(soundName) {
  stopSoundTrack()
  SOUNDTRACK = new Audio('game/sounds/'+soundName+'.mp3');
...
function dropPiece(event, square) {
  ...
  if (engine.getPiece(square) && valid) {
    ...
    document.getElementById(square).style.backgroundImage = pieceSelected;;
    playSound(valid);
    updatePgn();
  }
...
function think() {
  ...
  setTimeout(function() {
    ...
    if (engine.getPiece(targetSquare)) {
      document.getElementById(targetSquare).style.backgroundImage = pieceSelected;             
      playSound(bestMove);
      updatePgn();
      userTime = Date.now();
    }
...
function movePiece(userSource, userTarget) {
  ...
  drawBoard();
  var sourcePiece = document.getElementById(userSource);
  sourcePiece.style.backgroundImage = pieceSource
  sourcePiece.style.backgroundPosition = "center"
  sourcePiece.style.backgroundRepeat = "no-repeat";
}
...
function getGamePgn() {
  ...
  for (let index = 0; index < moveStack.length; index++) {
    ...
    updateGameScore(moveScore, moveDepth)
...
// update game score
function updateGameScore(moveScore, moveDepth) {
  var gameScore = document.getElementById('game-score');
  document.getElementById('game-depth').textContent = moveDepth
  score = (moveScore > 0) ? ('+' + moveScore/10) : moveScore/10;
  gameScore.textContent = score
  var scoreColor = (moveScore < 0) ? "red" : "black"
  gameScore.style.color = scoreColor
}
...
function downloadPgn() {
  ...
  header += '[Site "https://github.com/maksimKorzh/wukong-xiangqi"]\n';
